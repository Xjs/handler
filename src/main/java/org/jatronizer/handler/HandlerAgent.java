...
public class HandlerAgent implements ClassFileTransformer {
	...
	public HandlerAgent(HandlerSetup...setups) {
	...
	public HandlerAgent(Map<String, ASMTreeInstrumentation> instrumentationPlan) {
	...
	// for native methods (probably not that usable for Handler-Injection):
	...
	public static void agentmain(String agentArgs, Instrumentation inst) {
		...
		inst.addTransformer(new HandlerAgent(HandlerSetup.parseArgs(agentArgs)));
	}

	public static void usage(boolean argError) {
		if (argError) {
			System.err.println("missing handler class specification (agent args):");
		} else {
			System.err.println("This is not an executable jar, it is a library which makes Java more adaptable and flexible.\n"
					+ "Have a look at the API documentation. The concept is explained in handler.HandlerInstrumentation,\n"
					+ "but to use it you have to provide an interface (which should be annotated with handler.Instruments).\n"
					+ "Then, you can modify classes by loading them through handler.HandlerLoader.\n\n"
			);
			System.err.println("You may also use this as an agent and start another jar with:\n"
					+ "  -javaagent:JAR=ARGS\n"
					+ "whereas ARGS is:"
			);
		}
		System.err.println(
				"  one or more handler specifications (separated by ';')\n" +
						...
						"  handlees specified by handler.Instruments in the handler are automatically added\n" +
						"  examples:\n" +
						"    my.Handler\n" +
